# Go Cloud Functions on EdgeOne Pages - Chi 框架

一个基于 Next.js + Tailwind CSS 前端和 Go Chi 后端的全栈演示应用，展示如何在 EdgeOne Pages 上使用 Chi 路由器部署 Go 云函数，支持 RESTful API 路由。

## 🚀 特性

- **Chi 路由器集成**：轻量级、可组合的 Go HTTP 路由器，支持中间件栈和路由组
- **现代化 UI 设计**：深色主题搭配 #1c66e5 点缀色，响应式布局配合交互元素
- **交互式 API 测试**：内置 API 端点测试面板 — 点击 "Call" 实时测试每个 REST 端点
- **RESTful API 设计**：完整的 Todo CRUD 操作，使用结构化路由组（`/api/todos`）
...
│   ├── api.go                     # Chi 应用，包含所有 REST API 路由
│   ├── go.mod                     # Go 模块定义
│   └── go.sum                     # Go 依赖校验和
├── src/
...
所有 API 端点定义在单个 `cloud-functions/api.go` 文件中，使用 Chi 的路由组：
...
Go 后端使用 Chi 的标准模式 — 可组合路由、中间件和子路由器：
...
在项目根目录创建 `cloud-functions/api.go`，编写 Chi 应用：
...
[![Deploy with EdgeOne Pages](https://cdnstatic.tencentcs.com/edgeone/pages/deploy.svg)](https://console.cloud.tencent.com/edgeone/pages/new?from=github&template=go-chi)
